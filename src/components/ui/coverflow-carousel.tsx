...
export interface CoverflowSlide {
  /** Optional image. Omit it and `content` carries the card instead. */
  src?: string;
  alt?: string;
  /** Rendered inside the card when there is no image — this app has none. */
...
export function CoverflowCarousel({
  ...
  const dragRef = React.useRef<{
    ...
    v: number;
    t: number;
    /** Null until the first move decides whether this is a swipe or a scroll. */
  ...
  const paint = React.useCallback(() => {
    ...
    const flat = reducedMotion.current;
    ...
    cardRefs.current.forEach((card, index) => {
      ...
      const ramp = flat ? 0 : Math.pow(distance, falloff);
  ...
  const onPointerMove = (event: React.PointerEvent<HTMLDivElement>) => {
    ...
    // Decide once, on the first few pixels, whether this gesture is ours.
    // Claiming the pointer only after that keeps a vertical flick scrolling
    // the page instead of being swallowed by the carousel.
    ...
    >
      <div className="relative">
        <div
          ...
          className="cursor-grab overflow-hidden py-10 outline-none ring-brand-mid focus-visible:ring-2 active:cursor-grabbing"
          style={{
        ...
        {showNavigation && (
          <>
            <button
              ...
              onClick={() => nudge(-1)}
              className="absolute left-3 top-1/2 z-[200] -translate-y-1/2 rounded-full bg-surface/70 p-2 text-ink backdrop-blur transition hover:bg-surface"
            >
            ...
            <button
              ...
              onClick={() => nudge(1)}
              className="absolute right-3 top-1/2 z-[200] -translate-y-1/2 rounded-full bg-surface/70 p-2 text-ink backdrop-blur transition hover:bg-surface"
            >
      ...
      {showCaption && active?.title && (
        <div
          key={selected}
          className="mt-2 flex animate-[fadeIn_0.3s_ease-out] flex-col items-center px-6"
        >
          ...
          {active.meta && active.meta.length > 0 && (
            <dl className="mt-4 w-full max-w-[230px] text-[12px]">
              {active.meta.map((row) => (
